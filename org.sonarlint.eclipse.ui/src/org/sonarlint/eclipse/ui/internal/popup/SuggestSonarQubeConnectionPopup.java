...
public class SuggestSonarQubeConnectionPopup extends AbstractSuggestConnectionPopup {
  public SuggestSonarQubeConnectionPopup(String serverUrl, Map<String, List<String>> sonarToEclipseProjectMapping) {
    super(serverUrl, sonarToEclipseProjectMapping, false);
  }

  @Override
  protected String getMessage() {
    if (sonarToEclipseProjectMapping.keySet().size() > 1) {
      return "For the SonarQube server '" + serverUrl + "' there are multiple projects that can be "
        + "connected to local projects. Click 'More Information' to see them all. Do you want to connect and bind the project?";
    }
    ...
    if (mappedProjects.size() > 1) {
      return "For the SonarQube server '" + serverUrl + "' the project '" + projectKey
  ...
  protected void createContentArea(Composite composite) {
    ...
    addLinkWithTooltip("Connect", "Connect to SonarQube", e -> {
      close();
