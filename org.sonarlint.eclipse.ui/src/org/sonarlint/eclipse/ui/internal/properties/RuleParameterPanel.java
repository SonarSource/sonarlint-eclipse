...
public class RuleParameterPanel extends Composite {
  ...
  public void setDefaultLinkVisibility() {
    var allParamAreDefault = selectedRuleMetadata.getParamsByKey().entrySet().stream()
      .allMatch(paramEntry -> !selectedRuleConfig.getParams().containsKey(paramEntry.getKey())
        || Objects.equals(paramEntry.getValue().getDefaultValue(), selectedRuleConfig.getParams().get(paramEntry.getKey())));
