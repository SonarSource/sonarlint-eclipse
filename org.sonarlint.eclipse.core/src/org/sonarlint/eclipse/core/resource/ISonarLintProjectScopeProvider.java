 ...
 *  to determine both which resources should be excluded or explicitly included.
 *
 ...
 *  @since 10.5
 */
public interface ISonarLintProjectScopeProvider {
  /**
  ...
  /**
   *  This is used to provide explicit inclusions, can be both files and folders denoted by their
   *  path. It is also used for traversing a project's resources in order to stop and not visit a
   *  specific resource. In contrast to
   *  {@link ISonarLintProjectScopeProvider#getExclusions(IProject)} there might be specific
   *  inclusions that are located inside exclusions - e.g. generated code from ProtoBuf as it was
   *  done in SonarLint for Eclipse before. As an example, JDT will provide the directories of the
   *  source entries on the classpath, that hold the actual sources and not the compiled ones.
   *
   *  @param project to check
   *  @return explicit inclusions if there are any, an empty set otherwise
   */
  default Set<IPath> getExplicitInclusions(IProject project) {
    return Collections.emptySet();
  }
}
