...
public class SonarLintBindingsBot {

  private SWTBotView botView;
  private final SWTWorkbenchBot bot;

  public SonarLintBindingsBot(SWTWorkbenchBot bot) {
    this.bot = bot;
    ...
    try {
      tItem.getNode("SonarLint Bindings").doubleClick();
      this.botView = bot.viewByTitle("SonarLint Bindings");
    } catch (Exception e) {
  ...
  public void removeAllBindings() {
    try {
      Arrays.stream(this.botView.bot().tree().getAllItems()).forEach(item -> {
        item.contextMenu("Delete Connection").click();
        bot.button("OK").click();
      });
    } catch (Exception e) {
